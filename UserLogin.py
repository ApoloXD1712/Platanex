...
import UserMotorBD
import UserRegistrar
import tkinter as tk
import customtkinter

def mostrarLoginuser(on_return):
    db = DataBase()
    ...
    def ocultarLogin():
        root.withdraw()
        UserMotorBD.mostrarMotor(mostrarLog)
        
    def ocultarLoginR():
        root.withdraw()
        UserRegistrar.mostrarRegistro(mostrarLog)    
    ...
    def login():
        ...
        sql = "select * from login where usuario=%s and contraseña=%s and tipo=0"
    ...
    frame.pack(pady=90, padx=240, fill="both", expand=True)
    ...
    button1 = customtkinter.CTkButton(master=frame, text="Register", command=ocultarLoginR)
    button1.pack(pady=12, padx=10)

    button2 = customtkinter.CTkButton(master=frame, text="Regresar", command=volver)
    button2.pack(pady=12, padx=20)
